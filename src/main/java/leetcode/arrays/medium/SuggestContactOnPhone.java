...
public class SuggestContactOnPhone {

  public static void main(String[] args) {
    String[] a = {"sander", "amy", "ann", "michael"};
    String[] b = {"123456789", "234567890", "789123456", "123123123"};
    String p = "777";
    SuggestContactOnPhone searchContact = new SuggestContactOnPhone();
    System.out.println(searchContact.solution(a, b, p));
  }

  public String solution(String[] nameArr, String[] numArr, String p) {
    TreeSet<String> possibleContacts = new TreeSet<>();

    for (int i = 0; i < numArr.length; i++) {
      if (numArr[i].contains(p)) {
        possibleContacts.add(nameArr[i]);
      }
    }

    return possibleContacts.isEmpty() ? "No Contact" : possibleContacts.first();
  }
}
